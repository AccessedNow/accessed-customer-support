  ...
  protected async findFromPartyService(id: string, entityType: string) {
    ...
    const { data: entityExisting } = await firstValueFrom(
      this.httpService.get<any>(`${partyServiceUrl}/api/user/${id}`).pipe(
        catchError((error: AxiosError) => {
          this.logger.error(`Failed to fetch ${entityType} data: ${error.message}`);
    ...
    if (entityInDB) {
      ...
      for (const field of fieldsToCompare) {
        const partyValue =
          fieldMapping[field] !== undefined ? fieldMapping[field] : entityExisting.data[field];
      ...
      if (hasChanges) {
        ...
        return await this.repository.update(entityInDB._id.toString(), updateData);
      }
